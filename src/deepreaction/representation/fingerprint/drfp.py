...
class DrfpEncoder:
        ...
        for index, _ in enumerate(in_mol.GetAtoms()):
            for i in range(1, radius + 1):
                ...
                if smiles != "":
                    ...
                    if get_atom_indices:
                        atom_indices[smiles.encode("utf-8")].append(
                            set(amap.keys())
                        )

        if not root_central_atom:
            for key in atom_indices:
                atom_indices[key] = list(
                    set([frozenset(s) for s in atom_indices[key]])
                )
    ...
    def internal_encode(
        ...
        Tuple[
            np.ndarray, np.ndarray, Dict[str, List[Dict[str, List[Set[int]]]]]
        ],
    ]:
    ...
    def encode(
        ...
        for _, x in tqdm(
            enumerate(X), total=len(X), disable=show_progress_bar
        ):
            if atom_index_mapping:
