...
from deepreaction.representation.graph.graph_reprs_utils import AtomOriginType
...
class GlobalPool(nn.Module):
    ...
    def __init__(self, aggr: Literal["add", "mean", "max"] = "add"):
        ...
        if aggr not in AGGR_FNS:
            raise ValueError(f"Invalid aggr: {aggr}. Must be one of {list(AGGR_FNS.keys())}")
...
class AtomTypePool(nn.Module):
    ...
    def __init__(
        self,
        pool_type: Literal["reactants", "products", "dummy", "reactant_product"],
        ...
        if pool_type not in self.TYPE_MAPPING:
            raise ValueError(f"Invalid pool_type: {pool_type}. Must be one of {list(self.TYPE_MAPPING.keys())}")
        self.pool_fn = AGGR_FNS[aggr]
