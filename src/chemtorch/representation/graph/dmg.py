...
class DMG(AbstractRepresentation[Data]):
    ...
    def __init__(
        ...
        qm_featurizer: Optional[
            FeaturizerBase[Atom] | FeaturizerCompose
        ] = None,
        single_featurizer: Optional[
            FeaturizerBase[Atom] | FeaturizerCompose
        ] = None,
    ...
    def construct(self, smiles: str) -> Data:
        ...
        for i in range(n_atoms_reac):
            ...
            if self.single_featurizer is not None:
                single_f.append(
                    self.single_featurizer(mol_reac.GetAtomWithIdx(i))
                )
        ...
        for i in range(n_atoms_prod):
            prod_idx = ri2pi[i]
            f_atoms.append(
                self.atom_featurizer(mol_prod.GetAtomWithIdx(prod_idx))
            )
            if self.qm_featurizer is not None:
                qm_f.append(
                    self.qm_featurizer(mol_prod.GetAtomWithIdx(prod_idx))
                )
            if self.single_featurizer is not None:
                single_f.append(
                    self.single_featurizer(mol_prod.GetAtomWithIdx(prod_idx))
                )
            atom_origin_type.append(AtomOriginType.PRODUCT)
            compound_idx = (
                (prod_origins[prod_idx] + n_reactant_compounds)
                if prod_origins
                else 0
            )
            atom_compound_idx.append(compound_idx)
        ...
        data.atom_origin_type = torch.tensor(
            atom_origin_type, dtype=torch.long
        )
        data.atom_compound_idx = torch.tensor(
            atom_compound_idx, dtype=torch.long
        )
        data.edge_origin_type = torch.tensor(
            edge_origin_type, dtype=torch.long
        )
                        ...
                        for local_idx, orig_idx in enumerate(
                            ...
                            if graph_idx is not None:
                                if len(feature_shape) == 1:
                                    ...
                                    combined_features[graph_idx, :] = (
                                        feat_data["features"][local_idx, :]
                                    )
    ...
    def _create_component_graph(self, mol, atom_indices, compound_idx) -> Data:
        data = Data()
        x = [
            self.atom_featurizer(mol.GetAtomWithIdx(idx))
