...
class AttentionMessagePassing(MessagePassing):
    ...
    def forward(
        ...
        if debug:
            ...
            print("incoming_edges_list", incoming_edges_list.shape)
        ...
        if lol:
            ...
            # so, [#edges in whole batch, 1 edge, hidden_size]
        ...
        if debug:
            print("edge_attr_dense", edge_attr_dense.shape)
            print("mask_edge_attr_dense", mask_edge_attr_dense.shape)
        ...
        if debug:
            print("edge_attr_rearranged", edge_attr_rearranged.shape)
            print(edge_attr_rearranged[4:8])
        ...
        if debug:
            print("incoming_edges", incoming_edges.shape)
            print("mask", mask.shape)
            print(incoming_edges)
            for i, j in zip(row[:4], col[:4]):
                print(i, j)
            print(mask)

        # if lol:
        #     # Q: single edges, K: respective incoming edges, V: respective incoming edges
        #     edge_attr_dense_updated, attention_weights = self.attention_2(
        #         edge_attr_dense,
        #         incoming_edges,
        #         incoming_edges,
        ...
        if debug:
            print("edge_attr_dense_updated", edge_attr_dense_updated.shape)

        # unmask
        edge_attr_dense_updated = edge_attr_dense_updated[mask_edge_attr_dense]

        if debug:
            print("edge_attr_dense_updated_2", edge_attr_dense_updated.shape)
        ...
        if return_attention_weights:
            return aggregated_messages, attention_weights
