...
class RXNGPSLayer(MPNNLayerBase):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        self.num_heads = att_layer_cfg.num_heads
    ...
    def forward(self, batch):
        ...
        h_attn = self._sa_block(h_dense, attention_mask, mask)
        h_attn = h_attn[mask]
    ...
    def _create_attention_mask(self, batch):
        device = batch.x.device
        x_dense, _ = to_dense_batch(batch.x, batch.batch, fill_value=0)
        ...
        attention_mask.diagonal(dim1=1, dim2=2).fill_(True)
        attention_mask = torch.logical_or(
            attention_mask, is_reactant.unsqueeze(-1) & is_product.unsqueeze(1)
        )
        attention_mask = torch.logical_or(
            attention_mask, is_product.unsqueeze(-1) & is_reactant.unsqueeze(1)
        )
