...
class DeepRXNAttLayer(AttLayer):
    ...
    def __init__(
        ...
        self.reactants_attention = nn.MultiheadAttention(
            embed_dim=out_channels,
            num_heads=num_heads,
            dropout=dropout,
            batch_first=batch_first,  # data [num_batches, max_nodes_per_batch, hidden_size]
        )
        self.products_attention = nn.MultiheadAttention(
            embed_dim=out_channels,
            num_heads=num_heads,
            dropout=dropout,
            batch_first=batch_first,  # data [num_batches, max_nodes_per_batch, hidden_size]
    ...
    def forward(self, batch: Batch) -> Batch:
        ...
        updated_reactants, _ = self.reactants_attention(
        ...
        updated_products, _ = self.products_attention(
        ...
        if self.layer_norm:
            combined_features = self.ln(combined_features, batch.batch)
        if self.batch_norm:
        ...
        final_features, _ = self.global_attention(
