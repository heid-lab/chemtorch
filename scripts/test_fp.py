...
###############################################################
def custom_featurizer(base_featurizer, dim, radius):
    fpg = rdFingerprintGenerator.GetMorganGenerator(radius,fpSize=dim)
    ...
    def featurizer(item):
        ...
        if item is None:
            ...
            features.extend(list(np.array(fpg.GetFingerprint(item.GetOwningMol(), fromAtoms=[item.GetIdx()]))))
        return features
...
def main():
    ...
    atom_featurizer = custom_featurizer(base_atom_featurizer, 50, 2)
